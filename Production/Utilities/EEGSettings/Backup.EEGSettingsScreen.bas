...
GLOBAL gTCPIPFilename AS ASCIIZ * 256
...
FUNCTION PBMAIN()
    ...
    gBLFilename = "H:\EEGSettings\Default.BL"
    gAIBFilename = "H:\EEGSettings\Default.AIB"
    gEEGFilename = "H:\EEGSettings\Default.EEG"
    gTCPIPFilename = "H:\EEGSettings\TCPIPSettings.ini"
    gCurrentDirectory = EXE.PATH$
        ...
        CASE %WM_COMMAND
            ...
            SELECT CASE AS LONG CB.CTL
                ...
                CASE %IDC_CHECKBOX_ConnectToActiviewTCPIP
                    ...
                    IF (lResult = 1) THEN
                        CONTROL ENABLE hDlg, %IDC_BUTTON_TCPIPSettings
                    ELSE
                        CONTROL DISABLE hDlg, %IDC_BUTTON_TCPIPSettings
                    END IF
                CASE %IDC_BUTTON_TCPIPSettings
                    IF CB.CTLMSG = %BN_CLICKED OR CB.CTLMSG = 1 THEN
                        SHELL "TCPIPSettings.exe " + gActiviewFilename, 1, EXIT TO exitCode
                    END IF
                ' */
    ...
    IF (gDefaultFilename <> "") THEN
        ...
        IF (TRIM$(temp) <> "") THEN
            ...
            IF (cnt > 0) THEN
                FOR x = 1 TO cnt
                    lbItem = PARSE$(temp, x)
                    LISTBOX SELECT hDlg, %IDC_LISTBOX_AIB, VAL(lbItem)
                NEXT x
            END IF
        END IF
        ...
        IF (TRIM$(temp) <> "") THEN
            ...
            IF (cnt > 0) THEN
                FOR x = 1 TO cnt
                    lbItem = PARSE$(temp, x)
                    LISTBOX SELECT hDlg, %IDC_LISTBOX_AuxiliarySensors, VAL(lbItem)
                NEXT x
            END IF
        END IF
        ...
        IF (TRIM$(temp) <> "") THEN
            ...
            IF (cnt > 0) THEN
                FOR x = 1 TO cnt
                    lbItem = PARSE$(temp, x)
                    LISTBOX SELECT hDlg, %IDC_LISTBOX_BipolarLeads, VAL(lbItem)
                NEXT x
            END IF
        END IF
...
FUNCTION EEGListBox(BYVAL hDlg AS DWORD, BYVAL lID AS LONG, BYVAL lCount _
    ...
    SELECT CASE lRes
        ...
        CASE 2 '64 channels
            REDIM labels(64)
            gEEGFilename = "H:\EEGSettings\Default64.EEG"
        CASE 3 '128 channels
            REDIM labels(128)
            gEEGFilename = "H:\EEGSettings\Default128.EEG"
    END SELECT
...
SUB writeToConfigFile()
    ...
    END IF
    WritePrivateProfileString( "Experiment Section", "TCPIPSettingsFile", gTCPIPFilename, gINIFilename)
    ...
    WritePrivateProfileString( "OPTIONS", "SampleRate", TRIM$(temp), gTCPIPFilename)
...
#PBFORMS BEGIN DIALOG %IDD_DIALOG1->->
    ...
    DIALOG NEW hParent, "EEG Settings", 45, 57, 446, 435, TO hDlg
    ' %WS_GROUP...
    CONTROL ADD OPTION,   hDlg, %IDC_OPTION_32Channels, "32", 50, 57, 65, 15, _
        ...
        %WS_EX_LTRREADING
    CONTROL ADD OPTION,   hDlg, %IDC_OPTION_64Channels, "64", 50, 75, 65, 15, _
        %WS_CHILD OR %WS_VISIBLE OR %BS_TEXT OR %BS_AUTORADIOBUTTON OR _
        %BS_LEFT OR %BS_VCENTER, %WS_EX_LEFT OR %WS_EX_LTRREADING
    CONTROL ADD OPTION,   hDlg, %IDC_OPTION_128Channels, "128", 50, 93, 65, _
        15, %WS_CHILD OR %WS_VISIBLE OR %BS_TEXT OR %BS_AUTORADIOBUTTON OR _
        %BS_LEFT OR %BS_VCENTER, %WS_EX_LEFT OR %WS_EX_LTRREADING
    CONTROL ADD LISTBOX,  hDlg, %IDC_LISTBOX_EEGChannels, , 235, 43, 115, 85, _
        %WS_CHILD OR %WS_VISIBLE OR %WS_TABSTOP OR %WS_VSCROLL OR %LBS_NOSEL _
        OR %LBS_NOTIFY, %WS_EX_CLIENTEDGE OR %WS_EX_LEFT OR _
        %WS_EX_LTRREADING OR %WS_EX_RIGHTSCROLLBAR
    CONTROL SET COLOR     hDlg, %IDC_LISTBOX_EEGChannels, -1, %LTGRAY
    CONTROL ADD BUTTON,   hDlg, %IDC_BUTTON_EditEEG, "Edit EEG", 355, 73, 60, _
        25
    CONTROL ADD LISTBOX,  hDlg, %IDC_LISTBOX_AIB, , 35, 163, 115, 85, _
        %WS_CHILD OR %WS_VISIBLE OR %WS_TABSTOP OR %WS_VSCROLL OR _
        %LBS_MULTIPLESEL OR %LBS_NOTIFY, %WS_EX_CLIENTEDGE OR %WS_EX_LEFT OR _
        %WS_EX_LTRREADING OR %WS_EX_RIGHTSCROLLBAR
    CONTROL ADD BUTTON,   hDlg, %IDC_BUTTON_LoadAIB, "Load AIB", 155, 178, _
        60, 25
    CONTROL ADD BUTTON,   hDlg, %IDC_BUTTON_EditAIB, "Edit AIB", 155, 208, _
        60, 25
    CONTROL ADD LISTBOX,  hDlg, %IDC_LISTBOX_BipolarLeads, , 235, 163, 115, _
        85, %WS_CHILD OR %WS_VISIBLE OR %WS_TABSTOP OR %WS_VSCROLL OR _
        %LBS_MULTIPLESEL OR %LBS_NOTIFY, %WS_EX_CLIENTEDGE OR %WS_EX_LEFT OR _
        %WS_EX_LTRREADING OR %WS_EX_RIGHTSCROLLBAR
    CONTROL ADD BUTTON,   hDlg, %IDC_BUTTON_LoadBL, "Load BL", 355, 178, 60, _
        25
    CONTROL ADD BUTTON,   hDlg, %IDC_BUTTON_EditBL, "Edit BL", 355, 208, 60, _
        25
    CONTROL ADD LISTBOX,  hDlg, %IDC_LISTBOX_AuxiliarySensors, , 35, 278, _
        115, 85, %WS_CHILD OR %WS_VISIBLE OR %WS_TABSTOP OR %WS_VSCROLL OR _
        %LBS_MULTIPLESEL OR %LBS_NOTIFY, %WS_EX_CLIENTEDGE OR %WS_EX_LEFT OR _
        %WS_EX_LTRREADING OR %WS_EX_RIGHTSCROLLBAR
    CONTROL ADD LISTBOX,  hDlg, %IDC_LISTBOX_SampleRates, , 178, 278, 115, _
        85, %WS_CHILD OR %WS_VISIBLE OR %WS_TABSTOP OR %WS_VSCROLL OR _
        %LBS_NOTIFY, %WS_EX_CLIENTEDGE OR %WS_EX_LEFT OR %WS_EX_LTRREADING _
        OR %WS_EX_RIGHTSCROLLBAR
    CONTROL ADD BUTTON,   hDlg, %IDC_BUTTON_OKEEG, "Save to CFG", 245, 410, _
        70, 20
    CONTROL ADD BUTTON,   hDlg, %IDC_BUTTON_Close, "Close", 330, 410, 55, 20
    CONTROL ADD FRAME,    hDlg, %IDC_FRAME1, "EEG Channels", 35, 28, 385, 105
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL2, "AIB Channels", 35, 148, 100, 15
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL4, "Bipolar Leads", 235, 148, 100, _
        15
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL5, "Auxiliary Sensors", 35, 263, _
        100, 15
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL6, "Sample Rates", 178, 263, 100, _
        15
    CONTROL ADD FRAME,    hDlg, %IDC_FRAME2, "Make Selections Here", 20, 8, _
        420, 392
    ...
    CONTROL ADD BUTTON,   hDlg, %IDC_BUTTON_SaveDefaults, "Save defaults", _
        95, 410, 70, 20
    CONTROL ADD LISTBOX,  hDlg, %IDC_LISTBOX_ScreenLength, , 309, 278, 116, _
        85, %WS_CHILD OR %WS_VISIBLE OR %WS_TABSTOP OR %WS_VSCROLL OR _
        %LBS_NOTIFY, %WS_EX_CLIENTEDGE OR %WS_EX_LEFT OR %WS_EX_LTRREADING _
        OR %WS_EX_RIGHTSCROLLBAR
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL7, "Screen Length (secs)", 309, _
        263, 100, 15
    CONTROL ADD CHECKBOX, hDlg, %IDC_CHECKBOX_ConnectToActiviewTCPIP, "Use " + _
        "Actiview TCPIP Server", 25, 377, 155, 15
    CONTROL ADD BUTTON,   hDlg, %IDC_BUTTON_TCPIPSettings, "TCPIP Settings", _
        188, 375, 80, 20
    ...
    CONTROL SET FONT hDlg, %IDC_BUTTON_TCPIPSettings, hFont1
#PBFORMS END DIALOG
