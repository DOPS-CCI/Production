...
'#RESOURCE "fEEGKappaClockRMSv22.pbr"
%USEMACROS = 1
...
GLOBAL gSeconds, gMinutes, gTotalSeconds, ghPDThread AS LONG
GLOBAL gSampleCnt, gTotalSecondsPassedFlag AS LONG
    ...
    SELECT CASE AS LONG CB.MSG
        ...
        CASE %WM_COMMAND
            ...
            SELECT CASE AS LONG CB.CTL
                ' /* Inserted by PB/Forms 04-04-2014 14:08:53
                CASE %IDC_TEXTBOX_RMSWindow
                ' */
...
SUB processBuffer()
    ...
    'startDoubleQueue(gRunningRMSAVGQueue)
     ...
     RunningRMSAVGQueue.initializeQueue(36)
...
'                    CONTROL REDRAW hDlg, %IDC_LABEL_Status
        ...
        IF (gSeconds MOD 5 = 0 AND gSampleCnt = 0) THEN 'every 5 seconds
            ...
            IF (gTotalSecondsPassedFlag = 0) THEN
            ...
            IF (gTotalSeconds = gRMSWindow) THEN
                gTotalSecondsPassedFlag = 1
            END IF

            IF (gSeconds MOD 5 = 0 AND gTotalSecondsPassedFlag = 1 AND gSampleCnt = 0) THEN
                #DEBUG PRINT " "
                #DEBUG PRINT "5 seconds after " + STR$(gRMSWindow) + " seconds..."
                ...
                '================================================
                'added 4/4/2014 per Ross Dunseath
                'write out the RMS every minute to a file.
                '================================================
                IF (gTotalSeconds MOD 60 = 0) THEN
                    OPEN "fEEGKappaClockRMS.CSV" FOR APPEND AS #1
                    PRINT #1, DATE$ + " " + TIME$ +", " + STR$(threeMinuteRmsAvg)
                    CLOSE #1
                END IF
            ...
            IF (gTotalSecondsPassedFlag = 1) THEN
...
SUB loadDefaults()
    ...
    GetPrivateProfileString("OPTIONS", "RMSWindow", "1", temp, 2048, filename)
    CONTROL SET TEXT hDlg, %IDC_TEXTBOX_RMSWindow, temp
    gRMSWindow = VAL(temp)
...
SUB saveDefaults()
    ...
    CONTROL GET TEXT hDlg, %IDC_TEXTBOX_RMSWindow TO temp
    temp = TRIM$(temp)
    WritePrivateProfileString("OPTIONS", "RMSWindow", temp, filename)
...
#PBFORMS BEGIN DIALOG %IDD_DIALOG1->->
    ...
    CONTROL ADD FRAME,    hDlg, %IDC_FRAME1, "Additional Settings", 8, 234, _
        616, 136
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL1, "Min LowerThreshold:", 40, 299, _
        152, 24
    CONTROL ADD TEXTBOX,  hDlg, %IDC_TEXTBOX_MinLowerThreshold, "", 192, 299, _
        96, 24
    CONTROL ADD TEXTBOX,  hDlg, %IDC_TEXTBOX_MaxLowerThreshold, "", 192, 328, _
        96, 24
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL3, "Max LowerThreshold:", 40, 328, _
        152, 24
    CONTROL ADD TEXTBOX,  hDlg, %IDC_TEXTBOX_MaxUpperThreshold, "", 464, 328, _
        96, 24
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL4, "Max UpperThreshold:", 312, 328, _
        152, 24
    CONTROL ADD TEXTBOX,  hDlg, %IDC_TEXTBOX_MinUpperThreshold, "", 464, 299, _
        96, 24
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL5, "Min UpperThreshold:", 312, 299, _
        152, 24
    CONTROL ADD TEXTBOX,  hDlg, %IDC_TEXTBOX_RMSWindow, "60", 192, 268, 96, 24
    CONTROL ADD LABEL,    hDlg, %IDC_LABEL6, "RMS Window (sec):", 40, 268, _
        152, 24
    ...
    CONTROL SET FONT hDlg, %IDC_TEXTBOX_RMSWindow, hFont4
    CONTROL SET FONT hDlg, %IDC_LABEL6, hFont4
#PBFORMS END DIALOG
