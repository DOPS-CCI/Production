...
FUNCTION ClearAleaBuffer(BYVAL x AS LONG) AS LONG
    ...
    DIALOG SET TEXT hDlg, "Generate Random Numbers  clearing Alea buffer..."

    CONTROL DISABLE hDlg, %IDC_BUTTON_Generate
    CONTROL DISABLE hDlg, %IDC_BUTTON_Save
    ...
    DIALOG SET TEXT hDlg, "Genterate Random Numbers"

    CONTROL ENABLE hDlg, %IDC_BUTTON_Generate
    CONTROL ENABLE hDlg, %IDC_BUTTON_Save
                ...
                CASE %IDC_BUTTON_Save
                    IF CB.CTLMSG = %BN_CLICKED OR CB.CTLMSG = 1 THEN
                            ...
                            ELSE
                                ...
                                PRINT #1, "Sample Size " +   STR$(gSampleSize * 8) +  " bits"
                                PRINT #1, "Time, El Sec, Bitsum, Cum Z"
        ...
        CASE "GETRESULT"
            ...
            IF (gSaveToFile = 1) THEN
                IF (delimiter = "," OR delimiter = $TAB) THEN
                    ...
                    'PRINT #1, "Sample Size " +   STR$(gSampleSize * 8) +  " bits"
                    'PRINT #1, "Time, El Sec, Bitsum-ev, Cum Z"
                    ...
                    IF (numberFormat<> "ZSCORE") THEN
                        ...
                        FOR x = 1 TO gSampleSize
                           totalBits =  totalBits + sumBitsOfByte(gSamples(x))
                        NEXT x
                        PRINT #1, totalBits; delimiter;
                    ...
                    ELSE
                        ...
                        PRINT #1, FORMAT$(totalBits, "0000");",";
                        PRINT #1, FORMAT$(zScore, "0000.0000")
                    END IF
                    'CLOSE #1
                ELSE
                    OPEN gFilename FOR BINARY AS #21

                    PUT #2, gBinaryPos, gSamples()
                    gBinaryPos = SEEK(#1)
            ...
            IF (gSaveToFile = 0) THEN
                IF (delimiter = "," OR delimiter = $TAB) THEN
                    ...
                    ELSE
                        ...
                        'zStd = SQR(zN * 0.25)
                        'zScore = (zTotal - zEv)/zStd
                        MyString.Add(FORMAT$(totalBits, "0000"))
                        MyString.Add(delimiter)
