...
GLOBAL gSocket, gFramesProcessed AS LONG
GLOBAL DataQueue AS IQUEUECOLLECTION
GLOBAL tlStopReceive AS LONG
'GLOBAL pTcpClient AS IMSWinsockControl
GLOBAL gDataBuffer, gUnfinishedBuffer, gRecvBuffer AS STRING * 2048
...
FUNCTION socketConnect() AS LONG
    LOCAL WsaInfo     AS WsaData
    LOCAL Sock_Sa AS SOCKADDR_IN
    LOCAL lSock, lRet, lEvent AS LONG
    LOCAL temp AS STRING
    LOCAL sockinfo      AS sockaddr

    lRet = WSAStartup(%WINSOCK_VERSION, WsaInfo)

    IF (lRet <> 0) THEN
        MSGBOX "ERROR: WSAStartup failed."
        EXIT FUNCTION
    END IF

    lSock =   Socket (%PF_INET, %SOCK_STREAM, 0)
    IF (lSock < 0) THEN
        MSGBOX "ERROR: Opening socket"
        EXIT FUNCTION
    END IF

    Sock_sa.sin_family        =   %AF_INET
    'Sock_sa.sin_addr.s_addr   =   %INADDR_ANY                                                   'Accept all origin addr
    Sock_sa.sin_addr.s_addr   =   inet_addr( "10.10.11.11" )                                    'Accept all origin addr
    Sock_sa.Sin_port          =   HtoNS (9870)                                            '
    Sock_sa.sin_zero = STRING$(8, $NUL)


    lRet = Bind(lSock, Sock_sa, SIZEOF(Sock_sa))
    IF (lRet = %SOCKET_ERROR) THEN
        MSGBOX "ERROR: Could not bind socket."
        EXIT FUNCTION
    END IF

    lEvent = WSACreateEvent ()

    lRet = WSAEventSelect (lSock, lEvent, %FD_READ)
    IF (lRet = %SOCKET_ERROR) THEN
        MSGBOX "ERROR: Create event failed."
        EXIT FUNCTION
    END IF


    lRet = cconnect(lSock, Sock_sa, SIZEOF(Sock_sa))
    IF (lRet < 0) THEN
        MSGBOX "ERROR: Connect failed. " + STR$(WSAGetLastError())
        EXIT FUNCTION
    END IF

    DO WHILE (1)
        lRet = WSAWaitForMultipleEvents (1, VARPTR(lEvent), %True, 100, %False)
        IF (lRet = %WSA_WAIT_FAILED) THEN
            MSGBOX "ERROR: Wait failed."
        END IF

        WSAResetEvent(VARPTR(lEvent))

        IF (lRet = %WSA_WAIT_TIMEOUT) THEN       ' no data, loop back to beginning
            ITERATE DO
        END IF

        lRet = rrecv(lSock, gRecvBuffer, gRecvBufferLength, 0)
        IF (lRet = %SOCKET_ERROR) THEN 'recv2 data from socket
            MSGBOX "ERROR: recv failed."
            EXIT LOOP
        END IF

        temp =  VARIANT$(gRecvBuffer)
        DataQueue.enqueue temp

        IF (ghPDThread = 0) THEN
            tlStopReceive = 1 'Receive data and process
            THREAD CREATE ProcessDataThread(gRecvBufferLength) TO ghPDThread
        END IF
...
SUB parseBuffer()
    ...
    LOCAL char, recBuffer, temp, chStr AS STRING                                                              ' VOID
