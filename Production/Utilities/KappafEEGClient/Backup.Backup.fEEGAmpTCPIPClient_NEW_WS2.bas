...
SUB processData()
    ...
    OPEN "f:\debug_0816.txt" FOR OUTPUT AS #111
    sBuffer = ""
    WHILE(tlStopReceive)
        SLEEP 10
...
SUB parseBuffer()
    ...
    PRINT #111, " 2: " + STR$(gBytesInBuffer)

    nbrOfRecs = INT(bufLen / nbrOfBytes)
    bytesLeft = bufLen - (nbrOfRecs * nbrOfBytes)

    PRINT #111, " 3: " + STR$(nbrOfRecs)
    PRINT #111, " 4: " + STR$(bytesLeft)
            ...
            FOR x = 1 TO nbrOfRecs
                ...
                FOR y = 1 TO nbrOfChannels + 2
                    ...
                    IF (lResult = 1) THEN 'checked
                        ...
                        'END IF
                    END IF
                    PRINT #111, " 5: Frame : " + STR$(gFramesProcessed)
                    PRINT #111, " 6: Channel (" + STR$(y) + "): " + STR$(ch33.Channels(y))
...
SUB splitRecBufferIntoArray(recBuffer AS STRING, nbrOfRecs AS LONG, nbrOfChannels AS LONG)
    ...
    PRINT #111, " 4.1: " + STR$(recBufferlen)
    cnt = 0
    startPtr = 1
    ...
    DO
        ...
        IF (LEN(rec) <> endPtr) THEN
            PRINT #111, " 4.2: stop " + rec
        END IF
        gRecArray(cnt) = rec
        startPtr = startPtr + endPtr
        PRINT #111, " 4.3: " + STR$(startPtr)
