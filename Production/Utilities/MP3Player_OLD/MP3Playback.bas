...
#RESOURCE ICON,     MP3ICON, "MP3Player.ICO"
%USEMACROS = 1
            ...
            SELECT CASE LOWRD(CB.WPARAM)
                ...
                CASE %ID_TIMER02
                    IF (pauseStopFlag = 0) THEN
                        ...
                        CONTROL SET TEXT hDlg, %IDC_TEXTBOX_Position, SecondsToHHMMSS(position / 1000)
                        length = mp3.GetMediaLength()
                        CONTROL SET TEXT hDlg, %IDC_TEXTBOX_Length, SecondsToHHMMSS(length / 1000)
                        IF (position = length) THEN
                            CONTROL SET TEXT hDlg, %IDC_LABEL_Status, mp3Filename  + " has ended."
                            BEEP : BEEP : BEEP
                            KillTimer hDlg, %ID_TIMER02
                        END IF
                    END IF
...
#PBFORMS BEGIN DIALOG %IDD_DIALOG1->->
    ...
    CONTROL ADD TEXTBOX, hDlg, %IDC_TEXTBOX_Length, "0", 235, 125, 80, 20, _
        ...
        OR %ES_AUTOHSCROLL, %WS_EX_CLIENTEDGE OR %WS_EX_LEFT OR _
        %WS_EX_LTRREADING OR %WS_EX_RIGHTSCROLLBAR
    CONTROL ADD LABEL,   hDlg, %IDC_LABEL3, "hh:mm:ss", 135, 130, 95, 15
    CONTROL ADD LABEL,   hDlg, %IDC_LABEL5, "hh:mm:ss", 316, 130, 95, 15
