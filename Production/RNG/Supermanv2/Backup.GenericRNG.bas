...
SUB Response()
    ...
    EVENTSANDCONDITIONS(1).NbrOfGVars = 7
    ...
    IF (gTimerTix MOD 50 = 0) THEN
         ...
         ELSEIF (gHighOrLow = 2) THEN
            '#DEBUG PRINT "SLOW"
             giHPos = giHPos + 1
         END IF
         #DEBUG PRINT STR$(giHPos)
         'IF (iHPos = -176) THEN
         '    iHPos = 1200
         IF (giHPos >= 1200 + 260) THEN
             giHPos = -260
         END IF
...
SUB StartTrial()
    ...
    EVENTSANDCONDITIONS(0).NbrOfGVars = 7
...
FUNCTION EndTrial() AS LONG
    ...
    IF (globals.TrialCntTotal >= (globals.NbrOfRuns * globals.NbrOfTrials)) THEN
        ...
        SetMMTimerOnOff("DETERMINERESPONSE", 0)    'turn on
        killMMTimerEvent()
...
SUB WriteOutEvents()
  'TargetSelected
    ...
    EVENTSANDCONDITIONS(0).GVars(5).Condition = "RNGAverage"
    EVENTSANDCONDITIONS(0).GVars(5).Desc = LookupLegitimateGVDouble(EVENTSANDCONDITIONS(0).EvtName, EVENTSANDCONDITIONS(0).GVars(5).Condition, globals.RNGAverage)
    EVENTSANDCONDITIONS(0).GVars(6).Condition = "RunNumber"
    EVENTSANDCONDITIONS(0).GVars(6).Desc = LookupLegitimateGV(EVENTSANDCONDITIONS(0).EvtName, EVENTSANDCONDITIONS(0).GVars(6).Condition, globals.RunCnt)
    EVENTSANDCONDITIONS(0).GVars(7).Condition = "TrialNumber"
    EVENTSANDCONDITIONS(0).GVars(7).Desc = LookupLegitimateGV(EVENTSANDCONDITIONS(0).EvtName, EVENTSANDCONDITIONS(0).GVars(7).Condition, globals.TrialCnt)
   ...
   EVENTSANDCONDITIONS(1).GVars(0).Condition = "Target"
    ...
    EVENTSANDCONDITIONS(1).GVars(5).Condition = "RNGAverage"
    EVENTSANDCONDITIONS(1).GVars(5).Desc = LookupLegitimateGVDouble(EVENTSANDCONDITIONS(1).EvtName, EVENTSANDCONDITIONS(1).GVars(5).Condition, globals.RNGAverage)
    EVENTSANDCONDITIONS(1).GVars(6).Condition = "RunNumber"
    EVENTSANDCONDITIONS(1).GVars(6).Desc = LookupLegitimateGV(EVENTSANDCONDITIONS(1).EvtName, EVENTSANDCONDITIONS(1).GVars(6).Condition, globals.RunCnt)
    EVENTSANDCONDITIONS(1).GVars(7).Condition = "TrialNumber"
    EVENTSANDCONDITIONS(1).GVars(7).Desc = LookupLegitimateGV(EVENTSANDCONDITIONS(1).EvtName, EVENTSANDCONDITIONS(1).GVars(7).Condition, globals.TrialCnt)
