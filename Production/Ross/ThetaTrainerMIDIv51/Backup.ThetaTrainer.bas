...
SUB ButtonStartCLICKED()
    ...
    SELECT CASE gDefaultSound
        ...
        CASE 3 'mp3 chosen
            ...
            LOCAL pid AS DWORD
            pid??? = SHELL("MP3Playback.exe " + gMp3Filename, 6)
    END SELECT
END SUB
...
SUB ButtonMP3File_CLICKED()
    ...
    IF (TRIM$(gMp3Filename) <> "") THEN
        CONTROL SET TEXT hDlg, %IDC_LABEL_MP3File, PATHNAME$(NAMEX,  tempFilename)
        'LET gMp3 = CLASS "MyMP3Class"
...
FUNCTION cbHelperScreenEEGSettings() AS LONG
    ...
    SHELL "H:\EEGSettings2\EEGSettingsScreen.exe " + EXPERIMENT.ActiviewConfig + " " + filename, 1, EXIT TO exitVar
