...
FUNCTION LoadThetaTrainerSettings() AS STRING
    ...
    'and EMG saved.
    '=======================================================================
    GetPrivateProfileString( "Theta Settings", "EEGRMSMin", "", temp, %MAXPPS_SIZE, gIniFilename)
    sb.add("EEGRMSMin: " + temp + " " + $CRLF)

    GetPrivateProfileString( "Theta Settings", "EEGRMSMax", "", temp, %MAXPPS_SIZE, gIniFilename)
    sb.add("EEGRMSMax: " + temp + " " + $CRLF)

    GetPrivateProfileString( "Theta Settings", "EMGRMSMin", "", temp, %MAXPPS_SIZE, gIniFilename)
    sb.add("EMGRMSMin: " + temp + " " + $CRLF)

    GetPrivateProfileString( "Theta Settings", "EMGRMSMax", "", temp, %MAXPPS_SIZE, gIniFilename)
    sb.add("EMGRMSMax: " + temp + " " + $CRLF)
    ...
    SELECT CASE CBMSG
        CASE %WM_INITDIALOG
            IF (TRIM$(gHdrFilename) <> "") THEN
                CALL GetHeaderComment2(gHdrFilename)
...
CALLBACK FUNCTION cbControllerSave() AS LONG
    ...
    IF CB.MSG = %WM_COMMAND AND CB.CTLMSG = %BN_CLICKED THEN
            '...Process the click event here
        CALL ModifyHeaderComment2(gHdrFilename)
        CALL GetHeaderComment2(gHdrFilename)
