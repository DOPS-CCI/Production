...
#RESOURCE WAVE, NOISE2, "Sounds\YelpSiren500ms.wav"
#RESOURCE WAVE, NOISE3, "Sounds\ShotGunSound500ms.wav"
#RESOURCE WAVE, NOISE4, "Sounds\AAAAHHHHH500ms.wav"
...
SUB TrialChosen()
    ...
    #DEBUG PRINT STR$(globals.Response) + " " + PowerTimeDateTime(MyTime)
...
SUB TrialApproved()
    ...
    EVENTSANDCONDITIONS(1).NbrOfGVars = 4
    ...
    EVENTSANDCONDITIONS(1).EventTime = PowerTimeDateTime(MyTime)
        ...
        'PhotoDiodeOnOff(globals.hdl.DlgSubjectPhotoDiode,  1)
    bRndByte = AleaRNGByteRange(30, 255)
...
SUB WriteOutEvents()
  'RNGSelected
    ...
    EVENTSANDCONDITIONS(0).GVars(2).Condition = "SoundFile"
    EVENTSANDCONDITIONS(0).GVars(2).Desc = LookupLegitimateGV(EVENTSANDCONDITIONS(0).EvtName, EVENTSANDCONDITIONS(0).GVars(2).Condition, globals.SoundFile)
    ...
    EVENTSANDCONDITIONS(1).GVars(2).Condition = "SoundFile"
    EVENTSANDCONDITIONS(1).GVars(2).Desc = LookupLegitimateGV(EVENTSANDCONDITIONS(1).EvtName, EVENTSANDCONDITIONS(1).GVars(2).Condition, globals.SoundFile)
    EVENTSANDCONDITIONS(1).GVars(3).Condition = "RunNumber"
    EVENTSANDCONDITIONS(1).GVars(3).Desc = LookupLegitimateGV(EVENTSANDCONDITIONS(1).EvtName, EVENTSANDCONDITIONS(1).GVars(3).Condition, globals.RunCnt)
    EVENTSANDCONDITIONS(1).GVars(4).Condition = "TrialNumber"
    EVENTSANDCONDITIONS(1).GVars(4).Desc = LookupLegitimateGV(EVENTSANDCONDITIONS(1).EvtName, EVENTSANDCONDITIONS(1).GVars(4).Condition, globals.TrialCnt)
...
SUB DoTimerWork(itemName AS WSTRING)
    ...
    SELECT CASE itemName
        ...
        CASE "MATCH"
            ...
            CALL TrialApproved()
            bRndByte = AleaRNGByteRange(1, 4)
            SELECT CASE bRndByte
                CASE 1
                    ...
                    globals.SoundFile = 1
                CASE 2
                    PLAY WAVE "NOISE2", SYNCH
                    PLAY WAVE END
                    'PlaySound "NOISE2",  0, %SND_ASYNC + %SND_RESOURCE
                    globals.SoundFile = 2
                CASE 3
                    PLAY WAVE "NOISE3", SYNCH
                    PLAY WAVE END
                    'PlaySound "NOISE3",  0, %SND_ASYNC + %SND_RESOURCE
                    globals.SoundFile = 3
                CASE 4
                    PLAY WAVE "NOISE4", SYNCH
                    PLAY WAVE END
                    'PlaySound "NOISE4",  0, %SND_ASYNC + %SND_RESOURCE
                    globals.SoundFile = 4
            END SELECT
            CALL EndTrial()
            SetMMTimerOnOff("MATCH", 0)    'turn off
            SetMMTimerDuration("TRIALS", globals.Response * 1000)
            SetMMTimerOnOff("TRIALS", 1)    'turn on
        'CASE "SUBJECTDIODE"
